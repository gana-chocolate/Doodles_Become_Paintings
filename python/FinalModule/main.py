...
parser.add_argument('--save_latest_freq', dest='save_latest_freq', type=int, default=3000, help='save the latest model every latest_freq sgd iterations (overwrites the previous latest model)')
...
parser.add_argument('--test_dir', dest='test_dir', default='./userOutputu', help='test sample are saved here')
...
def tfmain():
    ...
    with tf.Session() as sess:
        model = pix2pix(sess, image_size=args.fine_size, batch_size=args.batch_size,
                        ...
                        checkpoint_dir=args.checkpoint_dir, sample_dir='sample')
...
if __name__ == '__main__':
    ...
    for filename in filenames:
        ...
        cv2.imwrite('input3/' + str(count9) + '.' + filename.split(".")[1], img9)
        count9 = count9 + 1
    ...
    for filename in filenames2:
        ...
        imgB = Image.open('output_Canny/' + str(count) + '_' + str(0) + '.' + filename.split(".")[1])
        imgA = Image.open('input3/' + str(count) + '.' + filename.split(".")[1])
        result.paste(im=imgA, box=(0, 0))
        result.paste(im=imgB, box=(256, 0))
        result.save('datasets/car2/' + filename + '.jpg')
    ...
    for filename in filenames3:
        img9 = cv2.imread('userOutput/' + filename, )

        cv2.imwrite('userResult/' + userFile + '.' + filename.split(".")[1], img9)
